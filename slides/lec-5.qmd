...
```{r, eval = FALSE}
library(readr)
read_csv("https://rmorsomme.github.io/website/projects/training_set.csv")
```
...
(identifiability -- advanced topic)
...
-   $X = \left[\left(\dfrac{\text{Girth}}{2}\right)^2 * \text{Height}\right]$ is our new predictor, which we construct from existing variables.

## 

To accomplish this, we simply create a new variable corresponding to $\left(\dfrac{\text{Girth}}{2}\right)^2 * \text{Height}$.
