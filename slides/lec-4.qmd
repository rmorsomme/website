...
-   Homework 1 will be graded today
...
#| out-width: "100%"
#| echo: false
d_m <- lm(hwy ~ cty, data = d) %>%
  augment()
d_m_small <- filter(d_m, cty > 28)

ggplot(d_m, aes(x = cty, y = hwy)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "pink") +
  geom_point(data = d_m_small, aes(y = .fitted), color = "maroon", shape = "triangle", size = 2) +
  geom_segment(data = d_m_small, aes(xend = cty, yend = .fitted), size = 0.5, color = "gray20")
```
:::
:::
...
Residuals can be used to assess linearity.
...
We denote least-square estimates with the symbols $\hat{\beta}_0$ and $\hat{\beta}_1$.
...
-   stick to the range of the data
    -   extrapolate with care, not like [this](https://www.cc.com/video/l4nkoq/the-colbert-report-science-catfight-joe-bastardi-vs-brenda-ekwurzel)
...
Exercise [7.21](https://openintro-ims.netlify.app/model-slr.html#chp7-exercises) - start by fitting the model in `R` with the following command. - skip part c
...
```{r}
lm(hwy ~ displ, data = d) %>% augment %>% .[[".resid"]] %>% .^2 %>% sum
```
...
$$
    \text{hwy} \approx \beta_0 + \beta_1 \text{cty}
$$ - residuals - least-square estimates - parameter interpretation - model comparison with $R^2$ - outliers
:::
